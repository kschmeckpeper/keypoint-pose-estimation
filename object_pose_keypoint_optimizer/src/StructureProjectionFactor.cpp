      ...
      lambda_(lambda)
{
    ...
    for (auto& key : landmark_keys) keys_.push_back(key);
...
gtsam::Vector
StructureProjectionFactor::unweightedError(const gtsam::Values& values, 
    ...
    if (H) {
        ...
        if (k_ > 0) H->push_back(Dcoefficients(values));
    }
...
void StructureProjectionFactor::weightJacobian(gtsam::Matrix& H) const {
    ...
    if (k_ > 0) H.bottomRows(k_) *= std::sqrt(lambda_);
}
...
StructureProjectionFactor::Dobject(const gtsam::Values& values) const {
    ...
    if (k_ > 0) H.bottomRows(k_) = Eigen::MatrixXd::Zero(k_, 6);

    return H;
}

Eigen::VectorXd
StructureProjectionFactor::Dlandmark(const gtsam::Values& values, size_t landmark_index) const {
